...
@pytest.mark.skipif(sys.version_info[:2] != (3, 6) or python_implementation() != 'CPython', reason='Only running pre-commit-hooks on newest tested python version, '
...
@pytest.mark.nocoverage
@pytest.mark.skipif(sys.version_info[:2] not in ((3, 6), (2, 7)) or python_implementation() != 'CPython', reason='Only testing build on 2.7 and 3.6')
def test_build():
